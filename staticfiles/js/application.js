...
var currentWizardTabIndex = 0,
    ...
    $team0avatar = $('input[name="teamform-0-avatar"]'),
    $team0name = $('input[name="teamform-0-name"]'),
    $team0background = $('textarea[name="teamform-0-background"]'),


    //$proposal = $('input[name="proposal"]'),
    $richProposal = $('textarea[name="proposal_rich"]'),
    ...
    isValidBiIntro = true,
    isValidBiTeam0Avatar = true,
    isValidBiTeam0Name = true,
    isValidBiTeam0Background = true,
    isValidFileLogo = true,
    isValidFileProposal = true,
    isValidRichProposal = true,
...
$(document).ready(function(){
       ...
       $img.trigger('click');
        ...
        $img.change(function() {
            if( $(this)[0].files[0].name.split('.').pop().toLowerCase() === 'pdf' ) {
                $preview.removeClass('has-danger').find('img').attr('src', '../../static/images/pdf-icon.jpg' );
    ...
    $(document).on('click', '.btn-add-row', function(e){
        var $container = $(this).parents('div.clone-wrapper').eq(0),
            $cloneMaster = $container.find('div.clone-master').eq(0),
            $clonedRow = $cloneMaster.clone(),
            prefix = $cloneMaster.data('prefix');

        $clonedRow.removeAttr('data-prefix');
        $clonedRow.removeClass('clone-master').addClass('clone-child');
        //$clonedRow.find('input,textarea,select').val('');
        $clonedRow.find('input,textarea').val('');

        $clonedRow.find('.input-group').removeClass('has-success');
        $clonedRow.find('.input-group').removeClass('has-danger');
        $clonedRow.find('.input-group').parents('div').find('.error-msg').remove();
        $clonedRow.find('.preview').removeClass('has-danger');
        $clonedRow.find('.btn-delete-row').removeClass('hidedelete');



        $clonedRow.find('select').val('select');
        $clonedRow.find('input[type="checkbox"],input[type="radio"]').removeAttr('checked');
        $clonedRow.find('.preview').eq(0).find('img').attr('src', '../../static/images/preview.jpg');


        $clonedRow.insertBefore( $(this).parents('p').eq(0) );

        updateIndex($container, prefix);
    });
    ...
    */
    ...
    $team0name.on('blur', function(e){
        if( $.trim( $(this).val() ) === '' ) {
            $(this).parents('.input-group').addClass('has-danger');
            if( !$(this).parents('.input-group').parents('div').eq(0).find('.error-msg').length ) {
                $('<span />').addClass('error-msg').html('This field is required.').insertAfter( $(this).parents('.input-group') );
            }
            isValidBiTeam0Name = false;
        }else{
            $(this).parents('.input-group').removeClass('has-danger').addClass('has-success');
            $(this).parents('.input-group').parents('div').eq(0).find('.error-msg').eq(0).remove();
            isValidBiTeam0Name = true;
    ...
    $team0background.on('blur', function(e){
        if( $.trim( $(this).val() ) === '' ) {
            $(this).parents('.input-group').addClass('has-danger');
            if( !$(this).parents('.input-group').parents('div').eq(0).find('.error-msg').length ) {
                $('<span />').addClass('error-msg').html('This field is required.').insertAfter( $(this).parents('.input-group') );
            }
            isValidBiTeam0Background = false;
        }else{
            $(this).parents('.input-group').removeClass('has-danger').addClass('has-success');
            $(this).parents('.input-group').parents('div').eq(0).find('.error-msg').eq(0).remove();
            isValidBiTeam0Background = true;
...
function updateIndex($container, prefix) {
    var total = $container.find('.clone-child').length + 1, // 1 is for the parent row
        index = 1; // 1 because 0 is always the parent row

    $( '#id_' + prefix + '-TOTAL_FORMS' ).val( total );

    console.log('no. of clones:', $container.find('.clone-child').length);

    $container.find('.clone-child').each(function(){
        var $row = $(this);
        console.log('current index:', index);

        $row.find('input,textarea,select').each(function(){
            console.log($(this).prop('nodeName'), 'old:', $(this).prop('name'));

            var tempindex = $(this).prop('name').match(/\d+/)[0],
                name = $(this).prop('name').replace( tempindex, index ),
                id = 'id_' + name;
        ...
        index = index + 1;
        console.log('after iteration index:', index);
    });
}
...
function wizardNextTab(step){
    ...
    if( currentWizardTabIndex === 0 ) { // validation for tab 1
        ...
        if( $team0avatar[0].files.length === 0 ) {
            $team0avatar.parents('div').eq(0).find('.preview').addClass('has-danger no-icon');
            if( !$team0avatar.parents('div').eq(0).find('.error-msg').length ) {
                $('<span />').addClass('error-msg text-center').html('This field is required.').insertAfter( $team0avatar.parents('div').eq(0).find('.preview') );
            }
            isValidBiTeam0Avatar = false;
        }else{
            $team0avatar.parents('div').eq(0).find('.preview').removeClass('has-danger');
            $team0avatar.parents('div').eq(0).find('.error-msg').remove();
            isValidBiTeam0Avatar = true;
        }
        ...
        $team0name.trigger('blur');
        $team0background.trigger('blur');

        if( !isValidBiName || !isValidBiEmail || !isValidBiLocation || !isValidBiIntro || !isValidFileLogo || !isValidBiTeam0Avatar || !isValidBiTeam0Name || !isValidBiTeam0Background) {
